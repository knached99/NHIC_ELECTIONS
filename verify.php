...
if(isset($_GET['vKey'])){
  ...
  $verifyAccount = $dbConn->query('SELECT verified, vKey FROM nhicVoting WHERE verified =0 AND vKey="'.$vKey.'"');
  if($verifyAccount -> num_rows == 1){
    $updateAccount = $dbConn->query('UPDATE nhicVoting SET verified=1 WHERE vKey="'.$vKey.'"');
    if($updateAccount){
